...
`0.9.0`, or `1.0.0`. Use patch by default for compatible work; use minor for a
new backward-compatible public capability and major for a breaking public
contract. Conventional Commit output is advisory. Release preparation updates
files without committing, tagging, or pushing. Submit those changes through a
normal PR because direct pushes to `main` are not allowed.
