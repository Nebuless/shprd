...
before `bun run release:prepare <X.Y.Z | patch | minor | major>`. The check
rejects versions below the Conventional Changelog recommendation but permits a
larger bump. Release preparation updates files without committing, tagging, or pushing. Submit those
