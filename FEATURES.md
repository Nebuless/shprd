...
SHPRD is a browser and PWA client for a running
...
- On Herdr 0.9.0 endpoints, each browser remembers its workspace, tab and pane
  ...
  **Local navigation** versus the legacy **Shared navigation** fallback.
  ...
  An empty session can create its first workspace directly from SHPRD.
...
Herdr reports recognized agents and their state, and SHPRD projects that
...
SHPRD adds a repository-scoped lifecycle view around Herdr workspaces:
...
SHPRD understands the repository-local
...
| Paseo hook | When SHPRD runs it | Working directory |
| --- | --- | --- |
...
For `setup`, `opened`, and `teardown`, SHPRD first looks for `paseo.json` in
...
| `SHPRD_HOOK_EVENT` | `worktree.created`, `worktree.opened`, `worktree.before_remove`, or `worktree.removed` |
| `SHPRD_HOOK_CHECKOUT_PATH` | Same target path exposed under a `SHPRD_`-prefixed alias |
| `SHPRD_HOOK_SOURCE_CHECKOUT_PATH` | Same source path exposed under a `SHPRD_`-prefixed alias |
...
For safety, SHPRD skips a run when the checkout is dirty or on a detached
...
only while the workspace is open in the current SHPRD connection.
...
- Installable as a standalone PWA from iOS/iPadOS Safari, macOS Safari, Chrome,
  ...
  `shprd` server process; it does not provide offline access.
...
- Connect to a remote Herdr with `--ssh-host`; SHPRD automatically forwards
...
- Check for SHPRD releases and perform a checksum-verified, one-click binary
