...
The APK starts with a bridge picker, then opens the same React interface from
the selected bridge in its native WebView. Live workspaces still require a
running SHPRD bridge reachable from the device. The APK does not start a local
