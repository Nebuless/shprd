...
On Android, a caller-supplied Dioxus navigation handler runs before the default
external-link path. SHPRD uses this to keep its selected HTTPS bridge inside
its native WebView; upstream otherwise calls Android's default browser for
HTTP(S) links.
