...
**Start SHPRD on the computer running Herdr, then open its browser URL.**
...
> **Finish one small section first.** With Herdr and SHPRD already installed,
...
Your browser / installed PWA
          ...
          | HTTP + WebSocket
          v
SHPRD (the command is still shprd)
          |
...
**Herdr owns the terminal sessions. SHPRD provides the browser interface.
Your agent is still a CLI running in a terminal.** SHPRD is not a model
...
1. Confirm that Herdr is installed and running on the host, with a project you
   ...
   [herdr.dev](https://herdr.dev). SHPRD does not start Herdr for you.
2. Install SHPRD using the
   ...
   and use `shprd.exe`. Prefer the Herdr plugin? See the
   [plugin instructions](./DEPLOYMENT.md#herdr-plugin).
3. Start SHPRD using the path for your installation:
   ...
   **Plugin installation:** follow the
   [plugin startup instructions](./DEPLOYMENT.md#herdr-plugin) to start SHPRD
   and obtain its login URL from the plugin panel or command log. The plugin
   does not add `shprd` to `PATH`; skip the standalone command above.
...
> **Direct and service startup differ.** Running `shprd` directly defaults to a
> local-only listener. `shprd service install` also binds loopback only and needs
> no application token. A bare reinstall resets a prior SHPRD Tailnet mode back
> to loopback. For private Tailnet access, use `shprd service install --tailscale`
> to detect a Tailnet IPv4 binding while retaining token authentication, or
> `shprd service install --tailscale-no-auth` to detect a Tailnet IPv4 binding
> and rely solely on private Tailnet ACLs. Neither command changes Tailscale
...
3. Create and open a documentation worktree. SHPRD starts it from the latest
...
workspace is open in the current SHPRD connection. Leave this off for your
...
1. Open SHPRD in the mobile browser and select your practice project and
...
**You are done when:** an SHPRD icon on your home screen opens the same
...
keep-alive mechanism**. Host sleep, a stopped SHPRD process, or a disconnected
...
changes the connection list available to other authenticated users. SHPRD
...
**For regular access from your own phone or computer, start with an SHPRD
Tailnet service.** If you can already SSH to the work machine and only need
access from another computer, SSH is another option. Keep Tailscale Serve and
Tailcat for setups that need their specific forwarding behavior.

| Your goal | Recommended route | What you need |
| --- | --- | --- |
| Use SHPRD on the computer running Herdr | Local SHPRD | No extra networking tool |
| Reach the workspace from your phone away from home | SHPRD Tailnet service | Host and phone in the same tailnet |
| Manage remote Herdr through local SHPRD | SSH profile / `--ssh-host` | Local Linux/macOS, Herdr already running remotely |
...
Hop B: SHPRD -> Herdr
       Local sockets / an SHPRD SSH profile
```

The Tailscale and Tailcat examples solve **hop A**, bringing your browser to
SHPRD. `--ssh-host` solves **hop B**, bringing SHPRD to remote Herdr. You can
combine them, but start with the hop you actually need.

#### Safety checks before connecting

1. Allow only your devices or people you fully trust. Access to the SHPRD UI
   is effectively terminal and file access as the user running SHPRD.
2. **The current implementation skips built-in authentication for listeners
   configured as `127.0.0.1`, `localhost`, or `::1`, even with
   `SHPRD_PASSWORD` set.** Forwarding one of these listeners makes the
   outer tunnel or proxy the access boundary. There is no extra SHPRD
   password gate.
...
### 4.2 Tailscale + Serve: an HTTPS forwarding option
...
Tailscale Serve on the work computer
          ...
          v
127.0.0.1:8787 -> SHPRD -> local Herdr
```
...
3. Follow the [access-control documentation](https://tailscale.com/docs/features/access-control)
   ...
   SHPRD node. Check whether existing broad rules still allow other access.
   Membership in the same tailnet does not automatically imply least
   privilege. These rules determine who gets full SHPRD access; do not
   enable Serve before confirming their scope.

#### Start SHPRD with a Tailnet listener

For a managed Tailnet listener, install the service with
`shprd service install --tailscale` to keep token authentication, or
`shprd service install --tailscale-no-auth` to rely solely on private Tailnet
ACLs. These commands detect a Tailnet IPv4 address and don't change Tailscale
configuration. Review ACLs first. The no-auth mode never creates a LAN or
public listener. With token mode, run `shprd auth token show` to print the
active token.

Run this on **the work computer running Herdr**. Port 8787 must be free. If
SHPRD already runs through the plugin or a user service, do not start a second
instance; use the existing-service instructions below instead.

```bash
shprd --host 127.0.0.1 --port 8787
```

On Windows, use `shprd.exe`, usually `./shprd.exe` when launching from
the current PowerShell directory. Leave this terminal running SHPRD.

> **This setup has no SHPRD login page under the current implementation.**
...
`~/.config/shprd/shprd.env` on Unix or
`%APPDATA%\shprd\shprd.env` on Windows. Preserve other necessary
...
> headers do not automatically give SHPRD per-person authorization or
> read-only roles. Mount the proxy at the domain root `/`; do not assume SHPRD
...
persistently; **it does not start SHPRD or prevent host sleep**. Once the
setup is verified, use the deployment guide to configure an SHPRD user service
...
#### Option A: local SHPRD, remote Herdr

Use this when code and agents run on a Linux work machine while your browser
runs on a Mac, for example. SHPRD SSH profiles and `--ssh-host` currently
require **SHPRD itself to run on Linux or macOS**. Windows SHPRD supports
...
2. In local SHPRD, add an SSH profile through the connection selector beside
...
conflict with an existing SHPRD instance.

```bash
shprd --ssh-host workbox --host 127.0.0.1
```
...
#### Option B: SHPRD already runs remotely; forward its web port

This differs from `--ssh-host`: both services stay remote, while system SSH
forwards one TCP port. Remote SHPRD should listen on loopback. This skips
SHPRD login, so remote admission relies on SSH authentication. Local processes
...
does not treat the experimental Tailcat web demo as a general-purpose SHPRD
proxy or assume that phones can run these CLIs.** Prefer Tailscale for regular
mobile access.

#### Connect only the SHPRD port

1. Run SHPRD on the work computer at `127.0.0.1:8787` and verify its local
   page. **This listener has no SHPRD login gate. The experiment relies
...
Visiting browser -> 127.0.0.1:18787 -> tailcat forward
                                 ...
                                 (direct or via DERP)
                                          |
Work host: Herdr <- SHPRD :8787 <- tailcat serve
```

**You are done when:** the other trusted computer can use SHPRD without a
...
| Who gets access | Tailnet identity and policy; no SHPRD login gate in this example | The connection address and optional client restrictions; no SHPRD login gate in this example |
...
1. On the **SHPRD host**, probe its HTTP service:
   ...
   that every Herdr feature works. For a refused connection, check the SHPRD
   process, port, and service status before changing the VPN.

2. Open SHPRD locally on its host, authenticating first for a non-loopback
...
**You are done when:** you can identify whether the failure is in the SHPRD
...
| SHPRD opens directly despite a configured password | Loopback listeners skip built-in authentication. For non-loopback listeners, use a fresh incognito window to rule out an existing cookie. |
...
share only redacted logs, OS and SHPRD versions, and the failing step.
...
**Next time you open SHPRD, take one small action: find an agent waiting for
