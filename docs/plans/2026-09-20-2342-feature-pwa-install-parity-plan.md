...
- QR camera-scan acquisition on Android (in-app camera vs OS-level scan routing into the Dioxus shell via an App Link) — v1 pairing is paste-only; R13's scan half is satisfied by this explicit deferral.
...
- **Resolved during planning (U5):** served standalone pages read `location.origin`, the desktop iframe path uses the bridge-published `shellHostUrl()`, and the switch target persists in `localStorage["shprd-host-url"]` (U5 owns the write); the iframe-vs-standalone reconciliation is specified in U5.
...
**Scope:** Add `scripts/gen-host-rules.ts` emitting three artifacts from one rule source: `server/src/generated/host-rules.json` (checked in), `web/src/generated/host-rules.ts`, and a plain-JS inlineable predicate module written into `crates/shprd-shell/assets/generated/` — `MOBILE_BOOTSTRAP` assembles its string by embedding that generated file (same include-time mechanism as `shell-controls.js`, keeping the shell's no-`__SHPRD_HOST_URL__` injection contract intact), and `shell-controls.js` inlines the same emitted module; the generator runs in the web build and its outputs are checked in so the cargo build consumes them without a Bun dependency; rewrite `HostUrl::parse` to consume the fixture with `#[test]` coverage over the vectors; `remoteHost.ts` consumes the generated module (keeping its public API); `MOBILE_BOOTSTRAP`'s `normalize` is replaced by the generated predicate + path auto-correction with visible "path removed" feedback; `shell-controls.js` consumes the same rule set at build. Rejection copy unchanged for query/credentials ("Enter an HTTP(S) origin without credentials, path, query or fragment" stays the secret-refusal contract); path inputs auto-correct per AE1.
...
**Scope:** `server/src/index.ts`: add unauthenticated `/manifest.webmanifest` route (server-generated, `cache-control: no-store`, name/short_name/id/scope/start_url/display/theme_color/background_color/icons incl. maskable 192+512; OpenChamber-parity overrides hook left for per-host naming). `static-files.ts`: immutable cache-control + ETag for hash-named assets; runtime gzip negotiation; index.html contract untouched. Access-control decision stated explicitly: the pre-auth carve-out covers `/manifest.webmanifest` plus the icon paths the manifest and head tags reference (192/512/maskable/apple-touch-180) — non-sensitive static assets; HTML, JS bundles, and all API/WS routes remain behind the auth gate; a static-files test asserts the icon paths return 200 without a session cookie (keeps the Lighthouse gate passable). Build: derive icon variants (192/512/maskable/apple-180) from one source via the existing icon pipeline in `gen-public.ts`'s walk; emit head tags (`theme-color`, `apple-mobile-web-app-*`) + `<link rel="manifest">` into the built `index.html` (Vite transform or a small postbuild step) — Dioxus `mobile_index()` split preserves them. Client: map `/api/health` + 401 + network failures onto the R7 taxonomy in a small connectivity module consumed by the update poll and banner.
...
**Test scenarios:** happy path — staged update: SW detects a new precache revision, toast appears once, accept lands on the new version, no double reload; error path — server down at launch renders the cached shell with the unreachable state (AE7); edge case — stale tab lazy-chunk 404 is absorbed by the precache hit (no crash reload); integration — budget gate asserts the precache set stays within the 12 MiB total envelope (the 650 KiB / 200 KiB gzip figures remain initial-JS gates, not precache caps), and the added manifest/SW/precache files are counted against the asset file-count ceiling.
...
**Scope:** New `useInstallPrompt` hook (capture + defer + persisted dismissal; the connect gate is set on the first U3 taxonomy reachable-ok result and persisted under a named `localStorage` key next to the dismissal key so AE5's cross-session behavior is unambiguous; hidden in standalone via `isStandaloneDisplay`); iOS A2HS coach card in the same slot; standalone host switcher — in a served tab or installed PWA the active origin is `location.origin`; inside the desktop iframe it comes from the bridge-published `shellHostUrl()` (`__SHPRD_HOST_URL__` — the shell deliberately does not inject it into standalone pages, per the existing shell test assertion); a switched host persists to `localStorage["shprd-host-url"]` with U5 owning the write; port ConfigMenu's switch/confirm/test UI out of its `parent !== window` gating; connectivity banner driven by U3's taxonomy with distinct copy per state (text-not-color-only per DESIGN.md).
...
**Scope:** Desktop: "Pair device" affordance in the ConfigMenu host section rendering a QR (qrcode dep, `toDataURL`) + copy-link of the validated origin. Android: "Paste link" path in the bootstrap form accepting the bare validated origin only — query-bearing input is rejected with the existing secret-refusal copy, identically to credentials and fragments (the settled query hard-rejection governs all four surfaces; the pairing-token query flow stays deferred per the Product Contract); clipboard read via existing `navigator.clipboard` patterns.

**Test scenarios:** happy path — desktop renders a QR encoding exactly the validated origin (inspectable string, no credentials/query) (AE9); error path — mobile paste of a host+path input auto-corrects and validates per AE1; edge case — clipboard permission denied falls back to manual typing; integration — golden vectors confirm query inputs are rejected identically in all four surfaces.
