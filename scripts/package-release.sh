...
case "$platform" in
  darwin-arm64)
    ...
    binary="$root_dir/server/shprd-darwin-arm64"
    ;;
  darwin-x64)
    build_script="build:darwin-x64"
    binary="$root_dir/server/shprd-darwin-x64"
    ;;
  linux-x64)
    build_script="build:linux-x64"
    binary="$root_dir/server/shprd-linux-x64"
    ;;
  linux-arm64)
    build_script="build:linux-arm64"
    binary="$root_dir/server/shprd-linux-arm64"
    ;;
  windows-x64)
    build_script="build:windows-x64"
    binary="$root_dir/server/shprd-windows-x64.exe"
    binary_name="shprd.exe"
    ;;
  windows-arm64)
    build_script="build:windows-arm64"
    binary="$root_dir/server/shprd-windows-arm64.exe"
    binary_name="shprd.exe"
    ;;
  *)
...
package_dir_name="shprd-$platform"
package_dir="$root_dir/dist/$package_dir_name"
versioned_archive="$root_dir/dist/shprd-v$version-$platform.tar.xz"
latest_archive="$root_dir/dist/shprd-$platform.tar.xz"
versioned_checksum="$versioned_archive.sha256"
latest_checksum="$latest_archive.sha256"
update_manifest="$root_dir/dist/shprd-$platform.update.json"
...
printf '%s\n' \
  "{\"schema\":1,\"name\":\"shprd\",\"version\":\"$version\",\"platform\":\"$platform\",\"archive\":\"$(basename "$latest_archive")\",\"sha256\":\"$archive_digest\"}" \
  >"$update_manifest"
