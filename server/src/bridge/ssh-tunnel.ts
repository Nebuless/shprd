...
export function classifySshTunnelFailure(
  ...
  if (
    ...
    return new SshTunnelError(
      "SSH host-key verification failed; verify the host outside SHPRD",
      false,
      "host-key",
      exitCode,
    );
...
export function assertSshTunnelPlatformSupported(
  ...
  if (platform === "win32") {
    throw new SshTunnelError(
      "SSH connections from Windows are not supported because SHPRD's stream-local forwarding cannot create a local Windows named pipe",
      false,
      "unsupported",
      -1,
    );
