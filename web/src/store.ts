  ...
  const promise = (async () => {
        ...
        if (response.ok) {
          ...
          if (healthMatchesUpdateVersion(health, expectedVersion)) {
            ...
            set({
              ...
              notice: {
                kind: "success",
                message: `SHPRD ${expectedVersion} is running`,
                detail:
    ...
    set({
      ...
      notice: {
        ...
        detail: `Could not verify SHPRD ${expectedVersion}. Reload the page after checking the server process.`,
      },
    });
  })().finally(() => {
    ...
    bridge.onControl((control) => {
      if (control.type === "pause_connection") {
        store.pauseConnection(
          control.reason ??
            "Another SHPRD client paused this connection. Resume when you want this browser to sync again.",
        );
      }
    });
    ...
    set({
      ...
      notice: granted
        ? {
            ...
            detail: "SHPRD will notify you when an agent task completes.",
            autoDismissMs: 5000,
          }
        : {
            kind: "error",
    ...
    try {
      ...
      if (body?.installed) {
        ...
        set({
          ...
          notice: {
            kind: "success",
            message: `SHPRD ${installedVersion} installed`,
      ...
      set({
        ...
        notice: {
          kind: "success",
          message: "SHPRD is already up to date",
        },
      });
    } catch (e) {
      set({
